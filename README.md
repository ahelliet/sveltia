...
- **Logo personnalisé de l'admin** (`logo_url`, top-level) : remplace l'icône par défaut de Sveltia CMS dans l'en-tête de l'admin, dès le chargement du CMS. Pointe pour l'instant vers `static/images/logo.svg`, une copie du logo Svelte utilisé comme placeholder (même fichier que `src/lib/assets/favicon.svg`) — remplace ce fichier par ton propre logo (SVG ou PNG) quand tu en as un, pas besoin de retoucher `config.yml`.
