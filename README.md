...
Pour "Page" ou "Article de blog", un widget `relation` Sveltia CMS (`collection: "pages"` ou `"posts"`, `value_field: "{{slug}}"`) permet de choisir l'entrée directement dans une liste plutôt que de taper une URL à la main — si cette page/cet article est renommé, le lien reste correct puisqu'il pointe vers l'entrée elle-même, pas vers une chaîne figée. Ces deux types ont aussi un champ **Ancre** optionnel, pour cibler une section précise de la page/l'article visé (ex : `contact`, sans le `#`). Le type **Ancre sur cette page** couvre un lien qui reste sur la page en cours (pratique pour un menu à une seule page) ; **URL personnalisée** couvre tout le reste (lien externe, `/blog`, `/`, ou une ancre tapée à la main).
