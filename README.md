...
### Workflow éditorial (relecture avant publication)

`publish_mode: editorial_workflow` (top-level, `static/admin/config.yml`) change la façon dont une sauvegarde arrive sur `main` : **jusqu'ici, chaque sauvegarde dans l'admin partait directement dessus** (comme un `git commit` + `git push` immédiat) — avec ce réglage, ce n'est plus le cas.

Concrètement, chaque nouvelle entrée (ou modification) crée sa propre branche (`cms/<collection>/<slug>`) avec une pull request GitHub associée ; les sauvegardes suivantes ajoutent des commits à cette même branche, rien ne touche `main` avant publication explicite. L'admin affiche ces entrées non publiées dans un écran "Workflow" (accessible depuis le menu), organisées en 3 statuts : **Brouillon** → **En révision** → **Prêt à publier** — les faire progresser change juste des labels sur la pull request GitHub. "Publier" fusionne la pull request dans `main` (l'entrée devient réellement visible sur le site après le prochain déploiement) ; "Supprimer" ferme la pull request sans rien fusionner. Ce mode nécessite le backend GitHub (déjà utilisé ici) — voir la [doc Sveltia CMS](https://sveltiacms.app/en/docs/workflows/editorial).

Utile pour se relire (ou se faire relire) avant publication, ou préparer un contenu à l'avance sans le rendre visible tout de suite. À désactiver (retirer la ligne) si ce round de relecture est superflu pour vous et que vous préférez repartir sur des sauvegardes directes sur `main`, comme avant.
...
Par défaut, l'aperçu de Sveltia CMS affiche les blocs de façon brute (markdown non rendu, pas de mise en page). `static/admin/preview.js` remplace cet aperçu par un rendu qui se rapproche de `src/lib/components/Blocks.svelte` (via `CMS.registerPreviewTemplate`, l'API de personnalisation de Sveltia CMS), stylé avec `static/admin/preview.css`. Comme sur le vrai site chaque page est entourée du header/footer (`+layout.svelte`), l'aperçu d'un article va chercher les données de "Navigation" et "Paramètres globaux" via `getCollection('_singletons', <nom>)` (l'API de Sveltia CMS pour lire une autre collection depuis un aperçu — `getCollection(<nom>)` seul échoue toujours pour un singleton : ce n'est pas une collection nommée dans `config.yml`, il faut passer par le nom réservé `_singletons` puis le nom du singleton en second argument, un détail non documenté par Sveltia CMS, confirmé en lisant son code source) et les affiche autour de l'article — sinon l'aperçu montrerait un article "nu" que le site ne produit jamais réellement. Comme l'admin n'a pas de build step, ce script utilise les globals `h()`/`createClass()` exposés par Sveltia CMS (pas de JSX) et charge `marked` par CDN pour rendre le markdown des blocs — c'est une **approximation** pour les blocs Image/Image+Texte (les composants shadcn-svelte `AspectRatio`/`Card` n'existent que dans l'app SvelteKit, donc leur effet visuel est reproduit avec les mêmes classes Tailwind sur des éléments simples). La galerie, elle, est un vrai carousel : `embla-carousel` (la même librairie qu'utilise `embla-carousel-svelte` sous le capot du composant `Carousel`) est chargé par CDN et initialisé sur le HTML rendu — drag, snap et boutons prev/next fonctionnent réellement, pas juste visuellement.
...
**Balises `<meta>` (`src/lib/components/SeoHead.svelte`)** : toutes les pages (accueil, pages, articles, liste des articles) rendent ce composant au lieu de poser un `<title>` à la main. Il pose, à partir des données ci-dessus : `<title>` (composé via `titleTemplate`/`buildPageTitle()` dans `src/lib/site.ts` — sauf sur l'accueil, qui n'a pas de titre de page propre, voir plus haut), `<meta name="description">`, `<meta name="robots">` (`noindex, nofollow` si `noIndex` est coché sur l'entrée *ou* si `robotsIndexing` est désactivé globalement), `<link rel="canonical">`, les balises Open Graph (`og:type`, `og:title`, `og:description`, `og:url`, `og:site_name`, `og:locale`, `og:image`), les Twitter Cards, et `google-site-verification` si renseigné. Les URLs (image, canonical) passent par `absoluteUrl()` (`src/lib/site.ts`) pour être absolues comme l'exigent Open Graph/Twitter — pas de résultat correct tant que `siteUrl` n'est pas renseigné (même avertissement que pour le sitemap/RSS).

**Données structurées JSON-LD (`src/lib/structuredData.ts`)** : un graphe `Organization`/`WebSite` (`buildSiteStructuredData()`) rendu une fois pour toutes dans `+layout.svelte` (donc présent sur chaque page — normal pour un schéma d'identité de site), à partir de `organizationName`/`organizationLogo`/`sameAs` — ne rend rien tant que `organizationName` est vide (un `Organization` sans nom serait pire que pas de données structurées du tout). Chaque article ajoute en plus son propre schéma `BlogPosting` (`buildArticleStructuredData()`, dans `blog/[slug]/+page.svelte`) avec titre, description, image, date de publication. `ldJsonScript()` échappe les `<` avant d'injecter le JSON dans un `<script type="application/ld+json">` (au cas où un titre contiendrait `</script>`).

**`<html lang>` dynamique (`src/hooks.server.ts`)** : `src/app.html` pose `<html lang="%lang%">` (un simple espace réservé, pas une variable SvelteKit), remplacé au moment du rendu par la vraie locale (`settings.seo.locale`, ex. `fr_FR` → `fr-FR`) — corrige un `lang="en"` resté figé alors que le site est en français (mauvais pour l'accessibilité — un lecteur d'écran choisit la mauvaise prononciation — et pour le SEO).
