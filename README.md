...
- `src/lib/components/PageContent.svelte` : rendu d'une page (titre + blocs + `<title>`), partagé entre `/<slug>` et `/` (voir "Page d'accueil éditable" ci-dessous) — pour ne pas dupliquer ce rendu entre les deux routes. Exception : le `<h1>` visible n'est pas affiché quand `page.isHomePage` est vrai — répéter le titre de la page (ex : "Accueil") juste au-dessus du contenu de l'accueil est redondant, alors que ce même titre sert de repère sur les autres pages. Le `<title>` (onglet du navigateur, SEO) n'est pas concerné, il reste défini sur toutes les pages y compris l'accueil. Même règle répercutée dans `PagePreview` (`static/admin/preview.js`).
