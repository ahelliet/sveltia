		...
		return blocks
			.map(function (block, i) {
				...
				if (block.type === 'documents') {
					var docs = Array.isArray(block.documents) ? block.documents : [];

					return h(
						'div',
						{ key: key, className: 'flex flex-col gap-3' },
						docs.map(function (doc, j) {
							return h(
								'a',
								{
									key: j,
									href: resolveImage(doc.file),
									target: '_blank',
									rel: 'noopener',
									className:
										'flex items-start gap-3 rounded-2xl border border-border bg-card p-4 shadow-md ring-1 ring-foreground/5 transition-colors hover:bg-muted'
								},
								h(
									'svg',
									{
								...
								h(
									'div',
									{},
									h('p', { className: 'font-medium' }, doc.label || ''),
									doc.description
										? h('p', { className: 'mt-1 text-sm text-muted-foreground' }, doc.description)
										: null
								)
							);
						})
